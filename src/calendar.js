(function(factory) {
    ...
    Calendar.prototype.init = function() {
        ...
        for (var i = 0; i < aCalendars.length; i++) {
            ...
            /* 默认设置属性-- 判断元素是否有该属性，如果没有则设置默认，有则不管*/
            if(!attribute(aCalendars[i],'format')){
              attribute(aCalendars[i],'format', 'yyyy-mm-dd')
            }
            // 默认设置每个input 都是范围插件
            attribute(aCalendars[i],'daterange', 'true')
        ...
        oCalenMask.onclick = function(){
          // 如果是时间范围，则点击蒙层判断选中两个值才能关闭
          if(self.currentNode.oldValue){
            if(self.currentNode&&attribute(self.currentNode, 'daterange') === 'true'){
              if(self.currentNode.oldValue.split(',').length >=2){
                hideCalen();
              }
            }
          }else{
    ...
    Calendar.prototype.createCalenList = function(data, setTitle) {
        ...
        for (var i = 0; i < dSun; i++) {
            ...
            // 设置选中高亮--只针对年月日，时分秒可能有问题需要修改
            if(self.currentNode&&self.currentNode.oldValue){
              var dayEleVal = attribute(dayEle, 'data-calen').replace(/\//g,'-');
              var cacheTarget = dayEleVal.split('-');
              if(cacheTarget[1]<10){
                cacheTarget[1] = '0'+cacheTarget[1]
              }
              if(cacheTarget[2]<10){
                cacheTarget[2] = '0'+cacheTarget[2]
              }
              dayEleVal = cacheTarget.join('-');

              if(self.currentNode.oldValue.indexOf(dayEleVal) > -1){
                dayEle.classList.add('markDate');
              }
            }
    ...
    Calendar.prototype.createHeader = function(cb) {
        ...
        // 判断如果有header 删除再追加
        var headerDom = document.querySelector('.calendar-header')
        if(headerDom) {oCalen.removeChild(headerDom)}
    ...
    Calendar.prototype.createWeek = function() {
        ...
        // 判断如果没有周dom 再追加
        var weekDiv = document.querySelector('.calendar-week');
        if(weekDiv){oCalen.removeChild(weekDiv)}
          ...
          node.onclick = function() {
            var classList = this.classList;
                if (classList.contains('prev-to-month')) {
                    ...
                    // 设置日期范围
                    if(date){
                      this.classList.add('markDate')
                      var markDateEle = calendarList.querySelectorAll('.markDate')
                      // console.log(markDateEle,self.currentNode.oldValue,'ccc')
                      if(attribute(self.currentNode, 'daterange') === 'true'){
                        if(markDateEle.length >2 || self.currentNode.oldValue&&self.currentNode.oldValue.split(',').length >=2){
                          self.currentNode.oldValue = undefined
                          self.changes();
                          // 去掉不是点击的标记的元素
                          for(var i = 0; i < markDateEle.length; i++){
                            if(markDateEle[i].innerText !== this.innerHTML){
                              markDateEle[i].classList.remove('markDate')
                            }
                          }
                        }
                      }else{
                      ...
                      if(self.currentNode.oldValue&&self.currentNode.oldValue.split(',').length<2){
                        cacheArr=[]
                        cacheArr.push(self.currentNode.oldValue,date)
                      }else{
                        cacheArr.push(date)
                      }
                    ...
                    if (self.hours) {
                        ...
                        if(attribute(self.currentNode, 'daterange') === 'true'){
                          ...
                          // 设置范围
                          self.currentNode.innerHTML = cacheArr
                          self.currentNode.value = cacheArr;
                          self.currentNode.oldValue = cacheArr.join(',');
                          self.changes();
                          // console.log(self.currentNode.oldValue,cacheArr,this.innerHTML,'aaa')
